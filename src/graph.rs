  ...
  E: Edge,
{
  ...
  pub fn compute_immediate_dominators(&self, root: usize) -> Result<FxHashMap<usize, usize>> {
    ...
    println!("dfs_number: {:?}", dfs_number);
    for &vertex in self.vertices.keys() {
      ancestor.insert(vertex, None);
      match dfs_number.get(&vertex) {
        Some(dfs_idx) => {
          label.insert(vertex, *dfs_idx);
        }
        None => {}
      }
    }
  ...
  pub fn is_acyclic(&self, root: usize) -> bool {
    ...
    fn dfs_is_acyclic<V: Vertex, E: Edge>(
      ...
      let successors_are_acyclic = graph.successors[&node]
        .iter()
        .all(|successor| dfs_is_acyclic(graph, *successor, permanent_marks, temporary_marks));
  ...
  pub fn dot_graph(&self) -> String {
    ...
    let edges = self
      ...
      .map(|e| {
        ...
        format!(
          "{} -> {} [label=\"{}\", style=\"{}\", color=\"{}\", fontcolor=\"{}\", penwidth=\"{}\"];",
          e.1.head(),
          e.1.tail(),
          label,
          style,
...
mod tests {
  ...
  fn test_compute_loop_tree() {
    ...
    let expected_loop_tree = {
      ...
      tree
        .insert_vertex(Loop::new(1, vec![1, 2, 3].into_iter().collect()))
        .unwrap();
      tree
        .insert_vertex(Loop::new(2, vec![2].into_iter().collect()))
        .unwrap();
      tree
        .insert_vertex(Loop::new(4, vec![4].into_iter().collect()))
        .unwrap();
